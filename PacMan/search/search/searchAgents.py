    ...
    def getSuccessors(self, state: Any):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                ...
                rez = ((next_node, successorVisitedCorners), action, 1)
                successors.append(rez)
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    while cornersNotVisited:
        heuristic_cost, corner = \
            min([(util.manhattanDistance(curent, corner), corner) for corner in cornersNotVisited])
        cornersNotVisited.remove(corner)
        curent = corner
        sumCost += heuristic_cost
    return sumCost
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    for foodPosition in food:
        distanceToFood.append(mazeDistance(position, foodPosition, problem.startingGameState))

        if cont == 4 and problem.heuristicInfo['wallCount'] > 20:
            break
